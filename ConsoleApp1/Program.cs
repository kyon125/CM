...
namespace ConsoleApp1
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            double typeAp = (double)typeA / (double)Type.Length;
            double typeOp = (double)typeO / (double)Type.Length;
            double typeBp = (double)typeB / (double)Type.Length;
            double typeABp = (double)typeAB / (double)Type.Length;
            double typeEp = (double)typeElse / (double)Type.Length;          
           
            //定義男女生總身高，以及正確填寫資料的男女生人數
            int girlCM = 0,boyCM = 0,man2 = 0, woman2 = 0;
            ...
            //計算男女生及全班平均身高
            float girlCMavenger = (float)girlCM /(float)woman2;
            float boyCMavenger = (float)boyCM / (float)man2;
            float allCMavenger = (float)(boyCM + girlCM)/ (float)(woman2+man2);
